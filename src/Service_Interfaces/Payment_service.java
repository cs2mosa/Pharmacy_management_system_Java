...
/**
 * The PaymentServiceInterface defines the contract for payment processing
 * services.
 * It includes methods for processing payments and generating receipts.
 * NOTE : Cancelled payments are not removed from the repository, but their
 * status is updated, and their ID is set to -1.
...
abstract interface PaymentServiceInterface {
    ...
    /**
     * Withdraws a payment from the repository using its unique identifier. marks
     * the payment with id of -1.
     * 
     ...
     * @return The status of the withdrawal operation. -1 if failed, 0 if
     *         successful.
     */
     ...
     * @param OrderId   The unique identifier of the order associated with the
     *                  payment.PaymentId is the same as OrderId.
     * @return The status of the payment processing operation. Payment id if
     ...
     *
     * @param paymentId The unique identifier of the payment for which the receipt
     *                  is generated.
     ...
     * @param value     The new value to be set for the specified field, only 3
     *                  queries are supported for now -> "amount" , "payday",
     *                  "paymethod".
...
public class Payment_service implements PaymentServiceInterface {
    ...
    public int WithdrawPayment(int PatientId, int PaymentId) {
        try {
            return Payment_Repository.GetInstance().DeletePayment(PatientId, PaymentId);// Payment successfully
                                                                                        // withdrawn
    ...
    public int ProcessPayment(int PatientId, int PaymentId) {
        ...
        if (tempPatient.GetBalance() >= temppay.getOrder().getTotalPrice()) {
            ...
            Admin.setTotalIncome(
                    Admin.gettotalIncome() + (tempPatient.GetBalance() - temppay.getOrder().getTotalPrice()));
...
/*
 * // Implementation for adding a payment
 * if(payment == null || Order_Service.getInstance().GetById(OrderId) == null ||
 * Patient_Repository.getInstance().GetPatient(PatientId) == null)
 * return -1; //failed
 * payment.setID(OrderId);
 * payment.setStatus("Pending");
 * payment.setOrder(Order_Repository.getInstance().GetById(OrderId));
 * payment.setAmount(Order_Repository.getInstance().GetById(OrderId).
 * getTotalPrice());
